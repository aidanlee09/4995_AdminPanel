...
export async function proxy(request: NextRequest) {
  ...
  if (user) {
    ...
    if ((profileError || !profile?.is_superadmin) && !isAlwaysAllowed) {
    ...
    if (!profile?.is_superadmin && !isLoginPage && !isAuthCallback && !isAlwaysAllowed) {
    ...
    if (isLoginPage && (profile?.is_superadmin || isAlwaysAllowed)) {
